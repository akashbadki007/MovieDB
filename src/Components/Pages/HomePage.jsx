...
function HomePage() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/movie/popular?api_key=${API_KEY}&language=en-US&page=${currentPage}`);
  ...
  return (
    <div className="container mx-auto px-[20px] sm:p-4">
      <h1 className="text-center sm:text-left text-2xl font-bold mb-6">Popular Movies</h1>
          ...
          <Link key={movie.id} to={`/movie/${movie.id}`} className="block">
            <div className=" bg-white rounded-lg box-shadow overflow-hidden img-section">
              <img
                ...
                alt={movie.title}
                // sm:w-[250px] sm:h-[280px] object-cover 
